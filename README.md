...
- トップページ (`/index.html`) の「選ばれる理由」3項目は、`feature-media-slot` を実画像に置き換えやすい構成です。実運用時は `assets/img/` に画像を配置し、該当スロットを `<img>` 要素に差し替え、あわせて `<figure>` の `aria-hidden="true"` を外して画像に適切な `alt` を設定してください。
